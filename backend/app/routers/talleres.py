...
from ..models import Taller, TallerDetalle, Corte, Item
from ..schemas import TallerCreatePayload, TallerOut

router = APIRouter(prefix="/talleres", tags=["talleres"])

@router.post("/", response_model=TallerOut)
def crear_taller(payload: TallerCreatePayload, db: Session = Depends(lambda: next(get_db()))):
...
def ver_calculo(taller_id: str, db: Session = Depends(lambda: next(get_db()))):
